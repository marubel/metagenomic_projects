---
title: "CM_shotgun_BantuRegion_RFC.Rmd"
author: "MAR"
date: "6/19/2019"
...
which(is.na(metadata_IDs_filtered$Bantu_region))
...
metadata_IDs_filtered$Region <- NULL
metadata_IDs_filtered$Region <- mgsub(metadata_IDs_filtered$Sampling.Site, eastcities, "East")
metadata_IDs_filtered$Region <- mgsub(metadata_IDs_filtered$Region, southcities, "South") 
metadata_IDs_filtered$Region <- mgsub(metadata_IDs_filtered$Region, northwestcities, "Northwest")
#Updated_CM_Metadata <- as.data.frame(t(Updated_CM_Metadata))
metadata_IDs_filtered$Bantu_region <- NA
metadata_IDs_filtered$Bantu_region <- with(metadata_IDs_filtered, ifelse(Population =="Bantu" & Region =="South", "Bantu_S", Bantu_region))
metadata_IDs_filtered$Bantu_region <- with(metadata_IDs_filtered, ifelse(Population =="Bantu" & Region =="East", "Bantu_E", Bantu_region))
metadata_IDs_filtered$Bantu_region <- with(metadata_IDs_filtered, ifelse(Population =="Bantu" & Region =="Northwest", "Bantu_NW", Bantu_region))

metadata_IDs_filtered <- rownames_to_column(metadata_IDs_filtered, var = "sampleID")

metadata_IDs_filtered2 <- metadata_IDs_filtered

metadata_IDs_filtered2 <- metadata_IDs_filtered %>% filter(is.na(Bantu_region))

newIDdrops <- c(metadata_IDs_filtered2$sampleID)

#newIDdrops <- which(is.na(metadata_IDs_filtered$Bantu_region)) #check for NAs, note for downstream steps
#newIDdrops <- names(newIDdrops)
rownames(metadata_IDs_filtered) <- metadata_IDs_filtered$sampleID
metadata_IDs_filtered = as.data.frame(t(metadata_IDs_filtered))
...
abundance_scaled$Bantu_region <- metadata_IDs_filtered[rownames(abundance_scaled), 'Bantu_region']

#abundance_scaled$Total_ANTS <- as.factor(abundance_scaled$Total_ANTS)
#abundance_scaled <- as.data.frame(abundance_scaled)
str(abundance_scaled$Bantu_region)
#Add positive control
abundance_scaled_poscntrl <- abundance_scaled %>%
  mutate(fake_taxon = add_noise(Bantu_region))
...
abundance_scaled_poscntrl$Bantu_region <- as.factor(abundance_scaled_poscntrl$Bantu_region)
#abundance_scaled_poscntrl <- as.matrix()

RF_abundance_scaled_poscntrl_DS = randomForest(Bantu_region ~ ., abundance_scaled_poscntrl,ntree=5001,sampsize=c(5,5,5),strata=abundance_scaled_poscntrl$Bantu_region)
...
ggtitle("Bantu_region RF Classification") + 
...
```{r plotz_RF, dev="svg"}
RF_state_classify_imp_sorted$features[1:10]

newtest <- abundance_scaled_poscntrl %>% select(
 `X....................CP002729.1.Escherichia.coli.UMNK88..complete.genome`,          
 `X..................CP010122.1.Escherichia.coli.strain.C5..complete.genome`,         
 `X..................Escherichia.coli.UMNK88`,                                        
 `X........................FP929032.1.Alistipes.shahii.WAL.8301.draft.genome`,        
 `X......................Alistipes.shahii.WAL.8301`,                                  
 `X..................CP010236.1.Escherichia.coli.strain.S42..complete.genome`,        
 `X..................CP010124.1.Escherichia.coli.strain.C5.plasmid.B..complete.genome`,
 `X................Helicobacter`,                                                     
 `X....................Odoribacter.splanchnicus`,                                     
`X....................Alistipes.shahii`,    
Total_ANTS)

new <- melt(newtest, "Total_ANTS")

mt <- ggplot(new, aes(Total_ANTS, value)) +
 geom_point() +
 theme_classic()+ #base_size = 40) +
 facet_wrap(~variable)+
 #scale_y_log10()+
 theme(axis.text.x=element_text(angle=45, hjust=1)) #+
 #scale_fill_manual(values=c("Negative" = "#ff9914", "Positive" = "#47a025")) #+ theme(axis.text.x = element_blank())

#mt                  
ggsave("Total_ANTS_shotgunRFC.svg", height= 25, width= 40, plot = last_plot(), dpi = 300)
